...
| `match_results` | fixture_id, ht_home, ht_away, ft_home, ft_away, aet_home, aet_away, pens_home, pens_away, settled_at — **[SUPERSEDED 2026-09-08]** the table is **fully bitemporal**, one row per *(fixture, source, revision)*, and its current-row key is `(fixture_id, source_id)` so two providers can disagree without one overwriting the other. `settled_at` is replaced by **`occurred_at`** (valid time, §2.1); `settled_at` was ambiguous under bitemporality. Adds `result_source ∈ {played, awarded}`, `is_trainable`, and the three provenance columns. `aet_*` is **cumulative at the end of extra time**. Authoritative: **`PHASE-0-SPEC.md` §13**. |
| `match_stats` | fixture_id, team_id, is_home, shots, shots_on_target, corners, yellow, red, possession, xg, xga, deep_completions · one row per team per match — **[SUPERSEDED 2026-09-08]** it is **one row per (fixture, source, revision) with home/away paired columns**, not one row per team. **No `team_id`** (a cross-table `CHECK` policing it was proven unsound), **no `is_home`** (duplicates `fixtures.home_team_id`), **no `xga`** (one team's xGA *is* the other's xG — two columns, one fact). `deep_completions` is deferred to the task that needs it. Authoritative: **`PHASE-0-SPEC.md` §13.3**. |
