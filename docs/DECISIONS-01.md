...
**football-data.co.uk** — `[REPORTED, notes.txt returned 503 during research]` free CSV downloads, 25+ leagues, deep history, odds from multiple bookmakers, and — critically — **explicit closing-odds columns** distinguished by a `C` in the column name (e.g. `B365CH` = Bet365 closing home). — **[VERIFIED 2026-09-08]** every claim here now confirmed against the live source, with three corrections: the working host is the **apex** `football-data.co.uk` (`www.` returns 503); the non-`C` columns are documented as **pre-closing**, not opening, so P0-11 ingests only the `C` columns; and the files also carry **match statistics** (shots, on target, fouls, corners, cards), which P0-11 ingests. Authoritative: **`PHASE-0-SPEC.md` §16**. Covers 1X2, over/under and Asian handicap. Licence terms are not clearly published; the files are offered as free public downloads.
...
3. **Forward capture — our own `odds_snapshots`, from week one.** Poll a live odds source (The Odds API, or Sportmonks' odds add-on) on our own cadence and store our own time series.3. **Forward capture — our own `odds_snapshots`, from week one.** Poll a live odds source (The Odds API, or Sportmonks' odds add-on) on our own cadence and store our own time series. — **[SUPERSEDED 2026-09-08]** the table is **`odds_series` + `odds_ticks`**, not `odds_snapshots` (**`PHASE-0-SPEC.md` §4.1**, **§14**). The strategy — manufacture our own closing dataset from week one — is unchanged and remains the central recommendation. After one season we own a proprietary closing-odds dataset for exactly our fixture universe, at exactly our granularity, that no vendor can revoke or reprice.
...
- **Bitemporal on every correctable fact.** Not just `known_at`, but full revision history: `match_results` and `match_stats` become append-only revision tables with `(known_at, revision, superseded_at)` and a "latest" view. Providers silently revise scores and xG; without this, point-in-time backtests are quietly wrong. — **[SUPERSEDED 2026-09-08]** there is **no `revision` column**. §2.2's implementation pattern is `known_at` + `superseded_at` alone, and neither `external_ids` (P0-06), `fixture_schedule` (P0-07) nor the P0-08 fact tables carries one; ordering is by `known_at`. The bitemporal requirement itself stands unchanged. Authoritative: **`PHASE-0-SPEC.md` §13.2**.
...
4. **Provider-neutral core**: internal UUIDs, `external_ids` with validity ranges, `raw_payloads` archive, adapter interface. No provider ID is ever a primary key. — **[SUPERSEDED 2026-09-08]** unchanged in substance, and now implemented. The adapter interface is the single-`fetch` envelope of **`PHASE-0-SPEC.md` §15.2**, not the five-method signature of `ARCHITECTURE.md` §4A. Canonical DTOs carry `provider_key` as identity *input* for the resolver — never a field named `id` — so a repository cannot mistake a provider string for a canonical UUID (§15.3).
...
10. **Odds capture begins in week one**, storing changes rather than polls.10. **Odds capture begins in week one**, storing changes rather than polls. — **[SUPERSEDED 2026-09-08]** unchanged in substance. Recorded precisely: change-only is **ingestion behaviour**, not a database constraint — no constraint can compare a row against its predecessor. P0-10/P0-11 enforce it; P0-14 asserts it (**`PHASE-0-SPEC.md` §6 rule 15**, **§14.7**).
