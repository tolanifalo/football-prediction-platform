...
| [docs/MODEL-BASELINE.md](docs/MODEL-BASELINE.md) | **Authoritative for the prediction engine**: the production/control/experimental configurations, Poisson formulation, data-cutoff and leakage rules, cold-start policy, scoreline truncation, backtest methodology and measured results. **Production is Poisson + 180-day time decay.** |
...
| [docs/UPCOMING-FIXTURES.md](docs/UPCOMING-FIXTURES.md) | **Authoritative for the football-data.org fixture provider** — a DIFFERENT service from football-data.co.uk. Provider choice, identity handling, what it can and cannot establish about fixture identity, and its limits. |
| [docs/PREDICTIONS.md](docs/PREDICTIONS.md) | **Authoritative for the prediction pipeline**: the persisted artifact, its temporal semantics (`data_cutoff` vs `known_at`), identity and idempotency, the production job, and what is deliberately not stored. |
| [docs/MULTI-SEASON-EVALUATION.md](docs/MULTI-SEASON-EVALUATION.md) | The evidence behind the production model: the same candidates over six seasons, with a development/held-out split. **Overturns the single-season decay result.** Its recommendation of 180-day decay was subsequently adopted — see `MODEL-BASELINE.md` §0. |
